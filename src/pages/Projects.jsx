...
function ProjectCard({ project }) {
  ...
  return (
    ...
    >
      <h2 className="project-title" style={{ fontSize: 18, marginBottom: 4 }}>
        <Link
          to={`/projects/${project.project_id}`}
          style={{ color: 'rgba(255,255,255,0.92)' }}
          className="hover:text-[rgba(99,179,237,0.95)] transition-colors"
        >
          {project.title}
        </Link>
      </h2>
      ...
      <div className="actions">
        <Link to={`/projects/${project.project_id}`} className="btn" style={{ fontSize: 13 }}>
          Read more
        </Link>
        {project.github_url && (
          <a
